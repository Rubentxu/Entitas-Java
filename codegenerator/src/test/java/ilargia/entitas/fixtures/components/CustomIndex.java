...
public class CustomIndex extends EntityIndex<TestEntity, IntVector2> {
    ...
    public CustomIndex(TestContext context) {
        super("MyCustomEntityIndex",
                context.getGroup(Matcher.AllOf(TestMatcher.Position(), TestMatcher.Size())),
                (e, c) -> {
    ...
    @EntityIndexGetMethod
    public Set<TestEntity> getEntitiesWithPosition2(IntVector2 position, IntVector2 size) {
        return getEntities(position);
    }
}
