...
class CollisionSensorSystemSpec extends Specification {
    ...
    @Shared SensorEntity collisionEnemy = entitas.sensor.createEntity().addCollisionSensor("Boss").addLink(player.getCreationIndex(),"Boss")
    @Shared SensorEntity collisionGround = entitas.sensor.createEntity().addCollisionSensor("Ground").addLink(player.getCreationIndex(), "Ground")


    void 'Detectar la colision de un elemento "objetivo" del escenario'() {            // 2
        given: 'un sensor de collision por defecto'                    // 3
        SensorEntity collisionEnemy = entitas.sensor.createEntity().addCollisionSensor("Boss").addLink(player.getCreationIndex(),"Boss")
