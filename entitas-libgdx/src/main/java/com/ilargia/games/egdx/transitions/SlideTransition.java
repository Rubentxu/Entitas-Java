...
public class SlideTransition extends RenderTransition {
    ...
    public SlideTransition(float duration, int direction, boolean slideOut, Interpolation easing, EGEngine engine) {
    ...
    public void render() {
        ...
        engine.batch.begin();
        engine.batch.draw(texBottom, 0, 0, 0, 0, w, h, 1, 1, 0, 0, 0, current.getWidth(), current.getHeight(), false, true);
        engine.batch.draw(texTop, x, y, 0, 0, w, h, 1, 1, 0, 0, 0, next.getWidth(), next.getHeight(), false, true);
        engine.batch.end();
    }
