package ilargia.egdx.logicbricks.gen.sensor;

import ilargia.egdx.logicbricks.component.sensor.AlwaysSensor;
import ilargia.egdx.logicbricks.component.sensor.MouseWheelSensor;
import ilargia.egdx.logicbricks.component.sensor.NearSensor;
import ilargia.egdx.logicbricks.component.sensor.PointerOverSensor;
import ilargia.egdx.logicbricks.component.sensor.RadarSensor;
import ilargia.egdx.logicbricks.component.sensor.RaySensor;

/**
 * ---------------------------------------------------------------------------
 * '<auto-generated>' This code was generated by CodeGeneratorApp.
 * ---------------------------------------------------------------------------
 */
public class SensorComponentsLookup {

	public static final int AlwaysSensor = 0;
	public static final int CollisionSensor = 1;
	public static final int DelaySensor = 2;
	public static final int Frequency = 3;
	public static final int Link = 4;
	public static final int Mode = 5;
	public static final int MouseWheelSensor = 6;
	public static final int NearSensor = 7;
	public static final int PointerOverSensor = 8;
	public static final int RadarSensor = 9;
	public static final int RaySensor = 10;
	public static final int totalComponents = 11;

	public static String[] componentNames() {
		return new String[]{"AlwaysSensor", "CollisionSensor", "DelaySensor",
				"Frequency", "Link", "Mode", "MouseWheelSensor", "NearSensor",
				"PointerOverSensor", "RadarSensor", "RaySensor"};
	}

	public static Class[] componentTypes() {
		return new Class[]{AlwaysSensor.class, ilargia.egdx.logicbricks.component.sensor.CollisionSensor.class,
				ilargia.egdx.logicbricks.component.sensor.DelaySensor.class, ilargia.egdx.logicbricks.component.sensor.Frequency.class, ilargia.egdx.logicbricks.component.sensor.Link.class, ilargia.egdx.logicbricks.component.sensor.Mode.class,
				MouseWheelSensor.class, NearSensor.class,
				PointerOverSensor.class, RadarSensor.class, RaySensor.class};
	}
}